...
CSV = pd.read_csv('SDW2023.csv')

# Obter a lista de IDs de usuário
user_list = CSV['UserID'].tolist()
...
valid_users = [user_info for user_id in user_list if (user_info := fetch_user_by_id(user_id)) is not None]

# Definir a chave da API OpenAI
openai.api_key = "chave_de_api_aqui"

# Função para gerar notícias AI
def generate_copy(user_info):
...
for user_info in valid_users:
    news_message = generate_copy(user_info)
    print(news_message)
